    ...
    db.getConnection((err, connection) => {
        ...
        connection.query(query, (err, rows) => {
            ...
            let triggers = (rows || []).map(tools.convertKeys).map(row => {
                ...
                row.formatted = util.format('%s days after %s', days, row.rule === 'subscription' ? row.columnName : (util.format('%s <a href="' + config.www.baseDir + '/campaigns/view/%s">%s</a>', defaultEventMap[row.column], row.sourceCampaign, row.sourceCampaignName)));
