...
module.exports.logout = (req, res) => {
    ...
    res.redirect(config.www.baseDir + '/');
};
...
module.exports.login = (req, res, next) => {
    ...
    passport.authenticate(authMode, (err, user, info) => {
        ...
        if (!user) {
            ...
            return res.redirect(config.www.baseDir + '/users/login' + (req.body.next ? '?next=' + encodeURIComponent(req.body.next) : ''));
        }
        req.logIn(user, err => {
            if (err) {
                return next(err);
            ...
            return res.redirect(req.body.next || (config.www.baseDir + '/'));
        });
    })(req, res, next);
};
